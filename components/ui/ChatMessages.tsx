  ...
  return (
    <div className="mb-20 px-5">
      {!loading && messages?.length === 0 && (
        <div className="flex flex-col justify-center items-center p-20 rounded-xl space-y-2 bg-orange-400 text-white font-extralight">
      ...
      {messages?.map((message) => {
        ...
        return (
          <div key={message.id} className="flex my-2 items-end">
            <div
              className={`flex flex-col relative space-y-2 p-4 w-fit line-camp-1 mx-2 rounded-lg ${
                isSender
                  ? "ml-auto bg-orange-400 text-white rounded-br-none"
                  : "bg-gray-100 dark:text-gray-100 dark:bg-slate-700 rounded-bl-none"
              }`}
            >
              <p
                className={`text-xs italic font-extralight line-camp-1 ${
